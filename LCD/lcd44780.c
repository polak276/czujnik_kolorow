...
//konwertuje liczbe float na string i ja wyswietla

void lcd_ftoa(float liczba_fl)
{

	//itoa((int)liczba_fl,temp1,10); //wyluskanie czesci calkowitej i wyswietlenie
	lcd_int(liczba_fl);

	lcd_str(","); // kropka

	if(liczba_fl<0) liczba_fl=liczba_fl*(-1.0);

	liczba_fl=(1000*(liczba_fl-(int)liczba_fl)); //wyluskanie czesci ulamkowej

	if((int)liczba_fl%2==1) liczba_fl=liczba_fl+1;

	liczba_fl=liczba_fl/10;

	if(liczba_fl<10) lcd_str("0");
